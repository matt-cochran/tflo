...
If your logic is well-understood, rarely changes, and performance-sensitive, you're better off writing a custom node in Rust and compiling it directly into the graph. tflo exposes the [`CustomNode`](https://github.com/matt-cochran/tflo/blob/main/tflo-core/src/custom_node.rs) trait exactly for this — it's in `tflo_core::prelude`. A `CustomNode` receives one `f64` per wired input and returns one `f64`; return `f64::NAN` to mean "still warming up":
...
impl CustomNode for SnrFilter {
    fn eval(&mut self, inputs: &[f64]) -> f64 {
        let value = inputs.first().copied().unwrap_or(f64::NAN);
