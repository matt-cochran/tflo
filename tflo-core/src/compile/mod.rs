...
impl<R> NodeOp<R> {
    /// The accumulator [`ScanCodec`](crate::comp::ScanCodec) for a checkpointed
    /// `scan`/`scan2` op, or `None` for any other op (and for plain,
    /// non-checkpointed scans). Used by the snapshot layer to serialize and
    /// restore scan accumulators.
    pub(crate) const fn scan_codec(&self) -> Option<&crate::comp::ScanCodec> {
        match self {
            Self::ScanF64(_, _, _, codec) | Self::Scan2F64(_, _, _, _, codec) => codec.as_ref(),
            Self::Prop(_)
            | Self::Const(_)
            | Self::MapF64(..)
            | Self::Map2F64(..)
